...
if __name__ == '__main__':
    socketio.run(app, debug=True, host='0.0.0.0', port=5000)    recipient_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    content = db.Column(db.Text, nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
...
def handle_message(data):
    try:
        ...
        return
    if not can_access_chat(current_user, recipient_id):
        logger.warning(f"⚠️ Попытка несанкционированного доступа от {current_user.username} к {recipient_id}")
        return
