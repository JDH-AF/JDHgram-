...
# 1. Секретный ключ (обязательно!)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'vash_luboy_secret_123')

# 2. Настройка пути к базе (вот здесь была ошибка)
basedir = os.path.abspath(os.path.dirname(__file__))
database_path = os.path.join(basedir, 'chat.db')

# Прямо прописываем путь, чтобы SQLAlchemy его точно увидела
...
# 4. СОЗДАНИЕ ТАБЛИЦ (Добавь это сразу после db = SQLAlchemy(app))
# Это гарантирует, что база создастся до того, как кто-то нажмет "Register"
